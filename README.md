...
4. Start Silverpine and open **Rumor Settings** from Modding Tools.

Only one shared copy of `ModdingTools.dll` should be installed.

## Lifecycle

The plugin installs its Harmony patches and registers both Modding Tools
entries synchronously in BepInEx `Awake`. Silverpine destroys the initial
...
The temporary settings window is a `ModToolBehaviour` and releases its own
session normally when it closes or is destroyed.
